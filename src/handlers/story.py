...
async def cb_name_done(call: CallbackQuery) -> None:
    ...
    # Склоняем имя в родительный («для Кости», не «для Костя»).
    # genitive() умеет работать и с пустой строкой.
    pretty = genitive(name) if name else "этого ребёнка"
    await call.answer(
        f"🌙 Для {pretty} сказка на сегодня уже сложена.\n\n"
        "Если хотите ещё одну — выберите другого ребёнка "
