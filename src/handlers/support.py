...
async def cmd_support(message: Message) -> None:
    await message.answer(
        "🕯 Напишите прямо в этот чат — что Вас тревожит, и мы обязательно "
        "поможем. Наша поддержка прочтёт Ваше письмо в течение суток "
        "и ответит сюда же.\n\n"
        "Возможно, ответ уже ждёт Вас в нашей <b>Помощи</b> — загляните "
        "туда, это часто бывает быстрее."
    )
...
async def forward_to_admin(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    await message.answer(
        "💌 Спасибо за Ваше письмо. Наша поддержка прочтёт его в "
        "течение суток и ответит сюда же.\n\n"
        "Если ждать не хочется — загляните в /faq, там собраны ответы "
