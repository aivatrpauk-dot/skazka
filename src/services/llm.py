...
async def generate_story(
    *,
    ...
    # working memory при генерации концовки.
    if is_channel_post:
        user_message += (
            "\n\nОДНО ВАЖНОЕ УТОЧНЕНИЕ ПРО ФИНАЛ ЭТОЙ СКАЗКИ.\n"
            f"«{child_name}» здесь — это имя ГЕРОЯ сказки, не имя слушателя.\n"
            "Эта сказка идёт в ОБЩИЙ детский канал, её читают сотни разных "
            "детей и их родителей перед сном.\n\n"
            "Поэтому финальное пожелание перед сном обращай ко ВСЕМ "
